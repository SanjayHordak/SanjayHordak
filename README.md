<h1 align="center">
  Hi 👋, I'm SanjayKumar S
</h1>

<h3 align="center">
  <img src="https://readme-typing-svg.herokuapp.com/?lines=Frontend+%2F+Full-Stack+Developer;React+%26+React+Native+Specialist;MERN+Stack+Developer;Learning+Spring+Boot+%26+MySQL;Building+Real-World+Projects&center=true&width=550&height=45">
</h3>

---

## 🚀 About Me

💡 **Frontend-focused Full-Stack Developer** with strong UI skills and a growing backend foundation.  
I specialize in building **modern, scalable, and user-friendly applications** and continuously upskill myself to become a **complete full-stack engineer**.

- ⚛️ Strong expertise in **React & React Native**
- 🌐 Backend development using **Node.js & Express**
- 🗄 Database experience with **MongoDB**
- ☕ Backend fundamentals with **Java**
- 🌱 Currently deep-diving into **Spring Boot & MySQL**
- 🔥 Passionate about real-world projects & clean code

---

## 🛠 Tech Stack & Tools

### ⚛️ Frontend
![React](https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![React Native](https://img.shields.io/badge/React_Native-61DAFB?style=for-the-badge&logo=react&logoColor=black)

### 🌐 Backend
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-000000?style=for-the-badge&logo=express&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=java&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white)

### 🗄 Databases
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)

---

## 💻 What I Bring to the Table

- ⚛️ Pixel-perfect, responsive UI development
- 📱 Cross-platform mobile app development
- 🌐 REST API design & integration
- 🔐 Authentication & backend logic
- 🗄 Database schema design & queries
- 🧠 Strong problem-solving mindset

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=SanjayHordak&show_icons=true&theme=tokyonight" height="165"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=SanjayHordak&layout=compact&theme=tokyonight" height="165"/>
</p>

---

## 🏆 GitHub Achievements

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=SanjayHordak&theme=tokyonight&no-frame=true&row=1&column=6"/>
</p>

---

## 🚀 Key Projects

- 🛒 **Diecast Car Selling Platform**  
  *Full-stack application with frontend, backend APIs & database*

- 🌐 **Personal Portfolio Website**  
  *Showcasing projects, skills & experience*

- 📱 **React Native Applications**  
  *Cross-platform mobile apps*

- ⚙️ **Backend APIs**  
  *Built using Node.js, Express & MongoDB*

---

## 🌱 Currently Learning & Improving

- ☕ **Spring Boot** (REST APIs, JPA, Hibernate)
- 🗄 **MySQL** (queries, relationships, optimization)
- 🧠 Backend architecture & best practices
- 🚀 Writing scalable, production-ready code

---

## 📫 Let’s Connect

- 💼 LinkedIn: *(add link)*
- 🌐 Portfolio: *https://portfolio-sanjay-orpin.vercel.app/*
- 📧 Email: *sanjay09082000@gmail.com*

---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=SanjayHordak&label=Profile%20Views&color=0e75b6&style=flat" />
</p>

⭐ *Always open to learning, collaboration, and exciting opportunities*

